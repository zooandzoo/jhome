<?php
class FilterInput extends BaseObject{
	...
	public function clean($source, $type = 'string')
	{
		...
		switch (strtoupper($type))
		{
			...
			case 'INTEGER':
				// 只保留第一组数字(可能包括'-'字符),如aa22cc33取得22
				preg_match('/-?[0-9]+/', (string) $source, $matches);
				$result = @ (int) $matches[0];
				break;
	
			case 'UINT':
				// 只保留第一组数字绝对值(可能包括'-'字符),如aa-22cc33取得22
				preg_match('/-?[0-9]+/', (string) $source, $matches);
